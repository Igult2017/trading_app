...
through the real `is_momentum_candle`. Times below are UTC; his chart is UTC+3.
"""
import datetime
...
from strategies.vix1_momentum import (
    is_momentum_candle, long_requirement, momentum_run, size_note, size_yardstick, veto_reason,
)
...
if not gbp or not xau:
    ...
    n = len(w) - 1
    s.check("XAU/USD 11 Sep 03:00 UTC ($18.32, 2.47x) now qualifies", is_momentum_candle(w, n, False, XAU), True)
    ...
    s.check("...and the margin alone is enough (memory switched off)",
            with_rule(0.2, 0, lambda: is_momentum_candle(w, n, False, XAU)), True)
    s.teeth("the margin", with_rule(0.0, 0, lambda: is_momentum_candle(w, n, False, XAU)) is False)

    w = at_candle(gbp, 10, 10)
    n = len(w) - 1
    y = size_yardstick(w, n, False, GBP)
    s.check("GBP/USD 10 Sep 10:00 UTC (9.9p, 2.25x) now qualifies", is_momentum_candle(w, n, False, GBP), True)
    ...
    n = len(w) - 1
    y = size_yardstick(w, n, False, GBP)
    s.check("GBP/USD 11 Sep 08:00 UTC (10.3p, 2.22x) now qualifies", is_momentum_candle(w, n, False, GBP), True)
    ...
    s.teeth("the memory", with_rule(0.2, 0, lambda: is_momentum_candle(w, n, False, GBP)) is False)
    ...
    n = len(w) - 1
    s.check("GBP/USD 10 Sep 11:00 UTC is refused", is_momentum_candle(w, n, False, GBP), False)
    ...
    n = len(w) - 1
    s.check("GBP/USD 10 Sep 12:00 UTC (17.6p) is refused", is_momentum_candle(w, n, False, GBP), False)
    ...
    for bars, sym, day, hour in ((gbp, GBP, 10, 16), (gbp, GBP, 10, 18), (gbp, GBP, 11, 1), (gbp, GBP, 11, 7),
        ...
        s.check(f"{sym} {day:02d} Sep {hour:02d}:00 UTC is refused", is_momentum_candle(w, n, False, sym), False)
    ...
    strict = {k for k in judged if with_rule(0.0, 0, lambda: is_momentum_candle(tail, k[0], k[1], GBP))}
    margin = {k for k in judged if with_rule(0.2, 0, lambda: is_momentum_candle(tail, k[0], k[1], GBP))}
    full = {k for k in judged if is_momentum_candle(tail, k[0], k[1], GBP)}
...
s.check("with only B inside C's window, C is REFUSED — a candle that needed the memory is never remembered",
        with_rule(0.2, 1, lambda: is_momentum_candle(chain, 102, True, GBP)), False)
s.teeth("the no-chaining rule", with_rule(0.2, 2, lambda: is_momentum_candle(chain, 102, True, GBP)) is True)
...
s.check("A 23 hours before B still counts",
        is_momentum_candle(series(BACK + [(0.0005, 100), (0.00055, 123)]), 101, True, GBP), True)
s.check("A 25 hours before B is ignored — B is judged on 2.3x alone and refused",
        is_momentum_candle(series(BACK + [(0.0005, 100), (0.00055, 125)]), 101, True, GBP), False)
