...
def check(symbol: str, direction: str, strategy: str,
    ...
    # RULE 8 IS GONE — the "a sell's stop must be worth 5x the spread" refusal. Removed on his
    # instruction, 2026-09-26: *"How about we just remove it because I have never understood its
    # essence"*, repeated as *"Can you first remove that spread gate"*.
    #
    # WHAT IT DID AND WHAT REMOVING IT COSTS, recorded so nobody has to re-derive it: measured over
    # 222 real fills with the broker's own bid AND ask ticks, the rule was worth +13.0R against
    # -21.6R without it (`docs/strategies/vix1-measured.md`). It refused 71 of 122 sells, and 7 of
    # those were winners worth +7.3R — his 22 Sep GBP/USD sell was one of them, refused at 4.3x and
    # it went on to reach its target.
    #
    # WHY REMOVING IT IS NOT SIMPLY GIVING THAT BACK. The rule was a filter on a symptom: sells only
    # failed that test because the STOP was tiny (the band it refused has a 4.2-pip median stop; the
    # band that survives has 9.3). The stop gap has been widened in the same change
    # (`strategies/vix1_entry._SL_GAP_MULT`) so the stops it was catching should no longer be
    # produced. If tiny-stop sells come back, this is the measurement that says they bleed —
    # bring back a rule, do not re-litigate the numbers.
