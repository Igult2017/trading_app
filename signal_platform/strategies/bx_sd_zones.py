...
# ── DID PRICE ACTUALLY PUSH AWAY FROM THE ZONE? ──────────────────────────────────────────────────
#
# HIS COMPLAINT, 2026-08-25, with charts: *"what those signals sent are candles in a ranging market,
# there was no movement there that would even cause break of structure... I am tired of receiving
# signals of miniature zones."*
#
# HIS RULE, from the marking images he supplied (Smart Risk, "Look for Strong Moves Away from the
# Zone"): *"The bigger and cleaner the move away from the zone, the stronger the imbalance."* The
# same sentence is the book's own definition (Ch.6 p25): *"Supply/demand is a zone, where price
# RAPIDLY PUSHES AWAY from (lots of orders placed), creating inefficiency (IFC), and breaks
# structure."*
#
# THAT SENTENCE WAS QUOTED AT THE TOP OF THIS FILE SINCE IT WAS WRITTEN AND NEVER IMPLEMENTED. A gap
# existing was the whole test. So a doji in a quiet range qualified exactly like a base under a
# 200-pip rally, which is what he has been receiving.
#
# TWO HALVES, BECAUSE HIS SENTENCE HAS TWO. "Cleaner" is the SHAPE of the departure — an unbroken run
# in the zone's direction. "Bigger" is how far that run carried price CLEAR of the zone.
#
# WHAT IS COMPARED, AND WHAT IS NOT. His standing rule: *"zones are not measured because they have
# distinct qualities."* The zone's own SIZE is not measured here and that idea was dropped. What is
# compared is the MOVE AWAY, which his image states comparatively ("bigger... stronger"), and it is
# compared against the pair's OWN average candle — never a pip count — so one rule covers GBP/JPY and
# EUR/USD without a per-pair table.
#
# MEASURED before choosing (126 GBP/JPY zones, 800 real H4 bars): clean run >=2 keeps 67%, >=3 keeps
# 43%; distance >=1.5x average candle keeps 88%, >=2.0x keeps 74%. The zone he reported
# (216.458-216.794) has a 2-candle run but travelled only 1.01x an average candle, so the DISTANCE
# half is what refuses it.
_DEPART_RUN  = 2     # candles closing the zone's way, none closing back inside it
_DEPART_AWAY = 1.5   # how far that run must carry price clear of the zone, in average candles


def departed_strongly(candles: list[Candle], ifc: int, top: float, bottom: float,
                      bull: bool) -> bool:
    """Did price leave this zone the way his images draw it — a clean run that went somewhere?

    `ifc` is the impulse candle's index; the departure is judged from it forward. Returns False when
    there are not enough bars yet to tell, which keeps a zone PENDING rather than admitting it on
    incomplete evidence — the same direction of error the rest of the registry takes.
    """
    from shared.candle_math import atr
    a = atr(candles, 14)
    if a <= 0 or ifc >= len(candles):
        return False

    # CLEANER — consecutive candles closing the zone's way that never close back inside it. A candle
    # closing back into the zone ends the run: that is price returning, not leaving.
    run = 0
    for c in candles[ifc:]:
        went_our_way = (c.close > c.open) if bull else (c.close < c.open)
        closed_back  = (c.close <= top) if bull else (c.close >= bottom)
        if not went_our_way or closed_back:
            break
        run += 1
    if run < _DEPART_RUN:
        return False

    # BIGGER — how far that run carried price CLEAR of the zone's far side, in average candles.
    seg = candles[ifc:ifc + max(run, _DEPART_RUN) + 1]
    away = (max(c.high for c in seg) - top) if bull else (bottom - min(c.low for c in seg))
    return away >= _DEPART_AWAY * a
