...
code, you can just borrow it. Avoid reinventing the wheel."* So:
    ...
    momentum      `vix1_momentum.is_momentum_candle` — HIS 2.5 rule, imported and NOT re-implemented. Not
                  `qualifies_for_trade`: that one carries the 15 Sep margin and memory, which qualify a
...
def market_awake(h1: list[Candle], tstate, retracement, symbol: str, look: int) -> str | None:
    ...
    NOTHING HERE IS INVENTED. The momentum candle is `is_momentum_candle` (his 2.5 rule, unchanged); a candle
