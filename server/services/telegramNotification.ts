...
export class TelegramNotificationService {
  ...
  // ✅ Make constructor private - use . create() instead
  private constructor() {}

  private async initialize(): Promise<void> {
    try {
      // Telegram is now always enabled (set at top of file)
      ...
      // Check if token is the exact correct format (46 chars for this bot)
      if (token.length >= 44 && token.length <= 50 && token.includes(':AA')) {
        console.log(`✅ Token format looks valid (${token.length} chars)`);
      } else {
      ...
      if (tokenParts.length === 2) {
        console.log(`Token format: ${tokenParts[0]}:${tokenParts[1]. substring(0, 5)}...  (${tokenParts[1]. length} chars)`);
      } else {
        console.log(`⚠️ Token has ${tokenParts.length} parts (expected 2)`);
      }
      ...
      // Token is valid, create the actual bot with polling
  ...
  async sendTradingSignalNotification(signal: any): Promise<void> {
    try {
      if (!this.isInitialized || !this.bot) {
        console.log('⚠️ Telegram bot not initialized, skipping signal notification');
        return;
      }
      ...
      if (subscribers. length === 0) {
        console.log('⚠️ No active Telegram subscribers for signal notification');
        return;
      }
...
// ✅ Initialize the service asynchronously at startup
