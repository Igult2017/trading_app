...
| Is price still filling the long candle that made this leg? | `vix1_void.not_filling` — his rule of 2026-09-20: no trade in the long candle's direction until the fill is over and **two** momentum candles close beyond each other | **ON since 2026-09-21**, main route only, refusal-only. **Scoped by his ruling the same day:** it stands aside for the first momentum candle after a trend is established (`vix1_void.proves_the_turn`), because that is his 2026-08-25 proof trade and it keeps **one** candle |
...
| `vix1_void.py` | **the liquidity void — may VIX.1 take the trade it is already about to take?** (added 2026-09-20, switched on 2026-09-21). `state()` reads the long candle that made the leg and returns `no-void` / `not-filled-yet` / `filling` / `resumed` / `broken`; `not_filling()` is the refusal reason the main route uses; `proves_the_turn()` carries his 2026-09-21 scope (his proof trade keeps ONE candle, joining a move keeps TWO); `break_of_a_fill()` is branch C and is **switched off**. It NEVER opens a trade and never touches an entry, a stop, a target or the 1M trigger — *"Who told you we change the entry?"* A void it cannot read comes back `no-void`, which allows: it may cost a trade on purpose, never by accident |
