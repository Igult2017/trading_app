...
| Is the market ranging or choppy? | `vix1_chop.not_tradeable` — his band idea (6+ crossings of the middle in 24 hours) AND the wander ratio (the Choppiness Index, >= 58), **held as a latched state** | **ON since 2026-09-21, and FINAL** — `vix1_bias` asks it before the trend is even read and returns on a reason; `vix1_preclose` asks the same function. New entries only. The older flip-count in `vix1_tradeable` was DELETED in the same change: one module owns this. See D42 |
| Is price still filling the VOID this move left? | `vix1_void.not_filling` — his rule of 2026-09-20: no trade in the void's direction until the fill is over and **two** momentum candles close beyond each other | main route only, refusal-only, and **silent unless a real void is there**. A void is a BAND of price, never a candle — see the module row below. The 2026-09-21 scope patch (`proves_the_turn`) is DELETED on his instruction *"integrate not patch"* |
| Is this pending turn the break of a void price was filling? | `vix1_void.break_of_a_fill` | **BUILT, SWITCHED OFF** (`_BRANCH_C = False`) — measured, it fires on almost every reversal and cannot reproduce his own example. See the fix log, 2026-09-21 |
...
| `execution/sizing.py` | **lots → the integer the wire wants, and the contract size is PER SYMBOL.** It held one constant, `LOT_UNITS = 100_000`, which is a **currency** lot; **a gold lot is 100 ounces**, so the first gold order went out 1,000× too large and cTrader refused it on 2026-09-01 (*"Order volume = 13000.00 is bigger than maximum allowed volume = 5000.00"*). Nothing could have caught it: the symbol list the order path fetches is `ProtoOALightSymbol`, which carries **no `lotSize` and no volume limits** (verified against the live account, 02 Sep — 1,940 symbols, XAUUSD came back with seven fields and none of them a size). `connection.load_symbol_spec` now asks for the FULL `ProtoOASymbol` (`ProtoOASymbolByIdReq`) and `build_stop` re-derives the volume from the broker's own `lotSize`; a per-instrument table (XAU 100, XAG 5,000) is the fallback when that one extra round trip fails, so a spec that cannot be read never takes a trade down. `clamp_to_broker` then fits the size to the broker's own step/min/max **before the request leaves** — over the maximum is REFUSED, never capped, because a capped size is not the risk that was asked for; quantising down to the step is the one adjustment made and it is logged, since on a coarse step it silently costs a quarter of the position. Guarded by `test_order_volume.py` |
| `data/fix_book.py` | **what we currently know about prices** — and the one place that decides whether the stream is ALIVE. `is_stale()` separates three states, not two: never connected, warming up, and gone quiet. It used to separate only two, because `age()` returns None both for *"no tick has ever arrived"* and *"the stream stopped"* and the check read `a is None or a > limit_s` — so a session was judged dead **in the same millisecond it was born**, and both watchers DM'd him a false alarm on every deploy AND every time a new position opened its own stream (measured 02 Sep: alarm at 03:56:11.854, stream opened 03:56:11.853, recovered 769 ms later). `connected` is now a property that stamps the clock, and silence with no ticks yet is measured from the moment the stream opened — the SAME 90-second limit, so a subscription that logs on and never delivers is still caught, just timed honestly. Guarded by `test_stream_grace.py` |
...
| `data/tick_bars.py` + `tick_bar_audit.py` | **CANDLES BUILT FROM THE FIX TICK STREAM - Phase 1, observing only** (added 2026-08-30). The broker publishes a finished bar **10-70s after it closes** (`candle_cache.py:45`, measured), which lands on the H1 momentum candle and then again on every M1 bar of the entry. A candle is only four numbers from a minute, so watching every price builds it ourselves and knows it finished at the second it finished. **H1 then comes from these via `candle_aggregator`, already proved 48/48 exact** - that note closed by naming this exact gap: *"nothing finer exists to build one from."* **BID TICKS ONLY** - cTrader's candles are bid-based (*"It is not possible to get trendbars based on ask prices"*, and measured 3-for-3 here). **TWO RULES CARRY THE SAFETY CASE:** a bar is kept only if the stream covered its ENTIRE minute (never a hybrid candle), and a symbol is served only after a run of EXACT matches with one mismatch withdrawing trust (never trusted without proof). cTrader publishes no guarantee that every tick is delivered, so correctness is never assumed. **Phase 1 serves NOTHING** - it compares and reports; the match rate is logged every 15 minutes and read with `tools/tick_bar_match.py` |
...
| `monitor/exit_watch.py` | **SAYS WHEN THE TRADE IS OVER** (added 2026-09-02) — his report: *"right now i dont know whether we are out or not"*. Nothing was watching the right thing: `signal_monitor` closes a signal on the SIGNAL's ORIGINAL stop/target, and for VIX.1 those never move (the state that advances a signal's stop is built for `bx_sd` only), while the ladder moves the REAL stop to breakeven, +1R and beyond. **So every exit at a moved stop happened at a price the signal's levels never see, and the position simply stopped appearing in `position_tracker`.** Keeps a last-seen snapshot per position and announces once when it vanishes, reporting the stop it was carrying and the best R it reached — and saying plainly that the level is the stop, not a confirmed fill. **A FAILED broker read (`None`, not `[]`) announces NOTHING**: telling him he is out of a trade he is still in would be the worst false alarm this platform can send, and that is the case its test guards hardest |
| `notifications/safe_notify.py` | **THE TRADE NEVER WAITS ON TELEGRAM** (added 2026-09-02) — his rule: *"Telegram is only for messages."* `dispatcher._send_text` retries 3 times with 5s sleeps against 5s client timeouts, so ONE message against a dead Telegram cost up to **~25 seconds** — and it was awaited BEFORE the amend, on the 0.5s watcher, inside a loop covering every open position. `tell()` caps a send at 3s and can never raise; `tell_soon()` does not wait at all and is what the lock/breakeven messages use, since the rung is decided by the broker's confirmation rather than by Telegram. Its detached task READS its own exception — a task whose exception nobody retrieves is swallowed by the event loop, which is exactly the shape of the copy-platform defect found the same day. The reports that genuinely need Telegram's answer (`check_fills`, `exit_watch`) run LAST, after every amend. Measured with Telegram stubbed to hang 30s: **0.00s** to the last stop move, against 10.0s before |
| `monitor/rungs.py` | **THE ONE LADDER** (added 2026-09-02) — every rung, per strategy, in one table. Created because there were TWO ladders with different numbers for the same trade: `position_tracker` broke even at 1R while `vix1_manage` did nothing below 2R, so the DM advising him and the code moving his stop could disagree |
| `monitor/position_tracker.py` | **the trade tracker** (added 2026-08-21) — the rungs now come from `rungs.py`; measured on the REAL position |
...
| `vix1_chop.py` | **THE RANGE / CHOP GATE — is this market worth trading at all?** (his band idea built 2026-09-14, switched on and completed 2026-09-21). Three parts: his box + crossings, the wander ratio (`100 x log10(sum of true ranges / box) / log10(24)`), and **the latch** — ON when both agree for two hours running, HOLDS through quiet patches, OFF only when a candle CLOSES outside the box it latched onto. ⚠ The latch is the load-bearing part: measured, no per-hour cut can separate his marked ranges from ordinary market (33.3–67.1 against 25.4–68.4). The state is replayed from the bars every scan, never stored. It refuses NEW ENTRIES ONLY and never touches an open position |
| `vix1_void.py` | **the liquidity void — may VIX.1 take the trade it is already about to take?** (added 2026-09-20, rebuilt as a ZONE 2026-09-21). **A VOID IS A BAND OF PRICE, NOT A CANDLE**: three-candle gaps (`shared/zone_detection.find_fvg_zones`) merged the standard `join_consecutive` way, joining only gaps left by a candle over HIS bar — more than twice a momentum candle, i.e. >5x the 100-bar median body — and the band must be >= 1x ATR(14) wide. `state()` returns `no-void` / `not-filled-yet` / `filling` / `filled` / `resumed`, all measured against the band so a fill can never exceed 100%; `not_filling()` is the refusal reason; `break_of_a_fill()` is branch C and is **switched off**. The first build measured one candle's body and got 47% of his real voids wrong. It NEVER opens a trade and never touches an entry, a stop, a target or the 1M trigger — *"Who told you we change the entry?"* A void it cannot read comes back `no-void`, which allows: it may cost a trade on purpose, never by accident |
...
| **Management** | the LADDER, **defined once in `monitor/rungs.py`** and read by both the advice path and the code that moves the real stop. **HIS NUMBERS, 2026-09-02 (revised the same day): 0.4R → breakeven (net-zero, not the entry) · 2.0R → lock +1R · then TRAILING — the stop keeps 0.1R behind, in 0.1R steps, until it is hit.** His words: *"when price moves to 2.1R lock 2R and when it moves to 2.5R lock 2.4R and when it moves to 2.6R lock 2.5R and go with that math until we are stopped out"*. The fixed 2.5R → lock 2R rung is GONE — the trail protects +2R from 2.1R, earlier and higher, and leaving it would have fired an alert at 2.5R telling him to move the stop DOWN. **ONLY BREAKEVEN AND THE EXIT ARE ANNOUNCED** — his rule of 2026-09-02: *"Locking Rs should only be announced when we move to breakeven and when we are out of the market... We dont need to get all the messages like 1R locked in the DM."* Every locking rung is `quiet=True`. **Quiet is about routine SUCCESS, never about failure:** `_auto_move` still speaks loudly whenever a stop does not reach the broker. **AND THE RUNG IS ONLY MARKED DONE ONCE THE BROKER CONFIRMS THE STOP IS THERE** — it used to be marked BEFORE the amend, so a refused or timed-out move was never retried and a "locked" R could be handed straight back to the market. A failure now leaves the rung unmarked and the next pass retries it (0.5s fast, 30s safety net); three failures escalate to one loud DM. The order's own take profit still performs the exit. Advice always; MOVED FOR HIM when `AUTO_BREAKEVEN_ENABLED` is on (off by default, demo-only, ratchet-only, both SL+TP legs always sent). **Per strategy** — a broker position carries no strategy, so `execution/fill_watch.owner_of` attributes it and anything unattributed keeps the older 1R/2R/3R/4R defaults |
