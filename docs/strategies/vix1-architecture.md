...
| **Management** | the LADDER, **defined once in `monitor/rungs.py`** and read by both the advice path and the code that moves the real stop. **HIS NUMBERS, 2026-09-02: 0.4R → breakeven (net-zero, not the entry) · 2.0R → lock +1R · 2.5R → lock +2R.** The order's own take profit still performs the exit. Advice always; MOVED FOR HIM when `AUTO_BREAKEVEN_ENABLED` is on (off by default, demo-only, ratchet-only, both SL+TP legs always sent). **Per strategy** — a broker position carries no strategy, so `execution/fill_watch.owner_of` attributes it and anything unattributed keeps the older 1R/2R/3R/4R defaults |
