...
| `vix1_chop.py` | **THE RANGE GATE — is price still inside the box the last move made?** (his rule, 2026-09-21). The two lines are the trend engine's own `bos_price` and `protected`; nothing is re-derived and **there is not one tuned number in the module** — the box is however big the last move was. A 24-hour band, a crossings count, the Choppiness Index and a latch were all built and DELETED here: he rejected them as complication and the measurements agreed (no cut could separate his marked range from ordinary market). Refuses NEW ENTRIES ONLY |
