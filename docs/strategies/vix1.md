...
| **the one refusal** | the FASTER structure (8-bar swings) must not be trending the OPPOSITE way. That is "the candle is in a pullback" |
| **maturity** | `developing` (first continuation after a range, pullback or reversal) and `developed` (one or more breaks behind it) are BOTH traded — the label is shown, never used as a gate |
