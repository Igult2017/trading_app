...
| **refusal 2** | a **developing** trend must show its **first pullback** before it is traded: the momentum candle has to come AFTER a retracement of one candle or more. **Length never disqualifies.** A **developed** trend is exempt — it has already proved itself, so a momentum candle anywhere along it is tradeable (added 2026-08-11) |
| **maturity** | `developing` (first HH+HL / LL+LH, nothing behind it yet) and `developed` (one or more breaks behind it). Both are TRADEABLE, but they are **not** treated alike — see refusal 2. *(This row read "the label is shown, never used as a gate" until 2026-08-11; that is no longer true.)* Read AT the momentum candle, not at the latest bar |
