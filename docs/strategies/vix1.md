...
- **`clear_trend` IS WINDOW-DEPENDENT. Its answer depends on the buffer size it is handed, not on
  the market.** It replays trend state from wherever the passed window happens to start, and the
  trend "persists until decisively broken" — so the starting point seeds the answer and then carries
  it. **Measured over 379 EUR/USD H1 timestamps: a 120-bar window and a 400-bar window agree only
  64% of the time, and EVERY disagreement is an outright opposite reading (+1 vs −1). 13 trend flips
  in 22 days at production's 120 vs 5 at 400.** Production passes `candle_counts[TF.H1] = 120`, so
  the live trend reading is one arbitrary choice among many. It is NOT what caused the 27 Jul missed
  signals — it read DOWN steadily all that day — which is why it was left alone. **Fixing it changes
  which trades fire everywhere**, so the user's decision (B1) was to document it and decide
  separately, against his own trade data, rather than as a side effect of a logging fix. The
  existing "KNOWN LIMIT: the verdict varies on 4% of bars across the 118-121 bars the feed delivers"
  in the `ea5c19c` row **understates this by an order of magnitude** — that measured jitter within a
  three-bar span, not sensitivity across realistic window sizes.
