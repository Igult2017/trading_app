...
### THE LEG RULE — a trend agreeing is not enough (settled 2026-08-11)
> "We can only take a trade after the first HH and HL (or LL and LH). Then when a momentum candle
> comes that forms the next HH (or LL), we take the trade — it is proof the trend is continuing. The
> CHoCH on the other hand tells us the trend has ended and the market is reversing so we take trades
> in the new direction, after LL and LH."
>
> "So, we don't trade pullbacks, we don't trade ranging markets and we are always in trend."

| | rule |
|---|---|
| **uptrend** | a confirmed **HH**, then a confirmed **HL** (the pullback has ENDED), then the momentum candle — which is the proof the next HH is developing |
| **downtrend** | a confirmed **LL**, then a confirmed **LH**, then the momentum candle |
| **after a CHoCH** | the count RESTARTS in the new direction — nothing is taken until that direction prints its own first pair |
| **refusals** | always stated in words: *"inside the pullback — the higher low has not formed since the high"* |

**TWO SCALES, AND DIRECTION NEVER COMES FROM THE FAST ONE.** The trend is read on 48-bar swings
(`vix1_bias._H1_SWING_N`); the leg is read on 8-bar swings (`vix1_structure._FAST_N`). This is not
tidiness — **half of all pullbacks are COMPLEX** (measured 2026-08-11: 21 of 42 on GBP/USD, 17 of 37
on EUR/USD over 12 months). A complex pullback prints its own LH+LL inside an intact uptrend and
looks exactly like a downtrend to any fast read. Let the fast read decide direction and the strategy
sells the pullback of a rally — precisely what it did on 10 Aug.

The trend's own width cannot do this job: at n=48 a higher low is confirmed 48 bars (two days) later,
by which time price has moved a median 92 pips and the leg is over.

**8 is provisional, by his decision:** *"keep it at 8 hours for now. I don't want to change that at
the same time as the trend logic. We can test 5, 8, and 12 hours later using the actual trading
results."* Gate opens 14.7% / 11.5% / 8.3% of the time at 5 / 8 / 12.
