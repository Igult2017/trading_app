...
- **Session System**: Session-based architecture. Users must create a trading session (with starting balance) before entering trades. `trading_sessions` table in DB (`shared/schema.ts`). All Dashboard, Metrics, Trade Vault, and Journal data is scoped to the active session via `sessionId`. Session CRUD at `client/src/components/CreateSession.tsx` (CreateSessionForm + SessionsList), API routes at `POST/GET/PUT/DELETE /api/sessions`. Active session stored in React state (`activeSessionId` in Journal.tsx), passed to all child components. Equity growth computed from risk% relative to session's starting balance via Python metrics calculator.
- **Journal Form**: 4-step trade logging wizard at `client/src/components/JournalForm.tsx`. Requires active session to save entries. Steps: Decision (thesis, triggers, impulse checks, rule governance), Execution (screenshots, position details, timing, timeframes, entry management), Context (market environment, TF context, liquidity, session timing, quality scores, technical signals), Review (exit causation, performance data, planning vs execution, trade metrics, psychology, reflections). Renders when sidebar nav `activeNav === 'journal'`.
...
- **Trading Calendar**: Monthly P&L calendar at `client/src/components/TradingCalendar.tsx` wired to `GET /api/calendar/compute?sessionId=X` via TanStack Query. Accepts `sessionId` prop — session-scoped. Python compute at `server/python/calendar_calculator.py` (CLI mode, spawned by `server/services/calendarCalculator.ts`) groups journal entries by date, returns daily P/L, trade count, win rate keyed by `"year-month"`. Day cells show profit/loss amounts, trade counts, win rates; hover tooltips; month navigation; stat cards (Net P&L, Win Rate, Total Trades, W/L Ratio); color-coded profit/loss bars; responsive layout. Uses Montserrat font. Shows "no session" prompt when none active. Renders when sidebar nav `activeNav === 'calendar'`.
...
- **Market Data**: Live price feeds for various asset classes via Python/yfinance integration. Historical OHLCV candles computed in `server/python/price_service.py` (spawned via CLI; reads JSON from `sys.argv[1]`).
- **Live Chart Streaming**: `TradingChart.tsx` connects browser-side directly to exchange WebSocket streams for tick-by-tick candle updates on crypto pairs (no server involved). Cascade: tries Binance (`wss://stream.binance.com:9443/ws/<symbol>@kline_<interval>`) first; if rejected within 4s (e.g. origin-blocked on dev domain), automatically falls back to Kraken (`wss://ws.kraken.com/v2` with OHLC subscription). Badge shows "LIVE · BINANCE" or "LIVE · KRAKEN". For forex/stocks/commodities, falls back to 30s polling via `/api/prices/:symbol/candles`. Note: Replit's screenshot/preview tool runs a headless browser on Replit's US servers, which is blocked by Binance; actual user browsers from non-blocked regions connect successfully.
