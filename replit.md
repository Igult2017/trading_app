...
- **Trade Vault**: Performance ledger at `client/src/components/TradeVault.tsx` with trade table (date, asset, strategy, session, outcome, P/L), summary stats (Net P/L, Win Rate, Trade Count), and inline edit modal for modifying trades. Uses JetBrains Mono + Montserrat fonts. Renders when sidebar nav `activeNav === 'vault'`.
