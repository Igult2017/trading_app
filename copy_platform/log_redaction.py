...
DELIBERATE TWIN of `signal_platform/core/log_redaction.py`. The two run as SEPARATE PROCESSES with
separate import paths, so neither can import the other; reaching across would tie the copy engine's
startup to the signal platform. If you change one, change both — the header below explains why this
exists at all.
