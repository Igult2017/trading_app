      ...
      {actionUser && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ ...cs, padding: '28px', maxWidth: '340px', width: '100%', border: `1px solid rgba(244,63,94,0.3)` }}>
            ...
            <p style={{ color: '#64748b', fontSize: '13px', textAlign: 'center', margin: '0 0 20px' }}>This will suspend <strong style={{ color: 'white' }}>{actionUser?.name ?? actionUser}</strong></p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              ...
              <button onClick={() => handleBanUser(actionUser?.userId ?? actionUser)} style={{ ...btn, padding: '10px', background: '#dc2626', color: 'white', border: 'none', fontSize: '13px' }}>Ban Account</button>
            </div>
          </div>
        </div>
      )}
