    ...
    <div style={{ minHeight: '100vh', background: pageBg, transition: 'background 0.3s' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '36px 28px 80px', display: 'flex', gap: 32, alignItems: 'flex-start' }}>
        ...
        <aside style={{ flexShrink: 0, width: 220, position: 'sticky', top: 100 }}>
          <div style={{ background: sidebarBg, border: `1px solid ${border}`, borderRadius: 12, overflow: 'hidden', marginBottom: 16 }}>
            <div style={{ padding: '14px 16px', borderBottom: `1px solid ${border}` }}>
              <span style={{ fontFamily: F, fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: textMut }}>Support</span>
            </div>
            {[
          ...
          <div style={{ background: sidebarBg, border: `1px solid ${border}`, borderRadius: 12, overflow: 'hidden' }}>
            ...
            {[
              ...
              <div key={label} style={{ padding: '12px 16px', borderBottom: `1px solid ${border}` }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* Hero text */}
          <div style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: '40px 48px', boxShadow: dm ? 'none' : '0 1px 4px rgba(0,0,0,0.05)', marginBottom: 24 }}>
          ...
          {/* FAQ section */}
          <div id="faq" style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: '40px 48px', boxShadow: dm ? 'none' : '0 1px 4px rgba(0,0,0,0.05)', marginBottom: 24 }}>
            <h2 style={{ fontFamily: F, fontSize: 18, fontWeight: 700, color: textPrim, margin: '0 0 6px', letterSpacing: '-0.02em' }}>
              Frequently Asked Questions
            </h2>
            <p style={{ fontFamily: F, fontSize: 12, color: textMut, margin: '0 0 28px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {faqs.map(({ q, a }, i) => {
                const isOpen = open === i;
                return (
                  <div key={i} style={{ border: `1px solid ${isOpen ? (dm ? 'rgba(37,99,235,0.4)' : '#bfdbfe') : border}`, borderRadius: 10, overflow: 'hidden', transition: 'border-color 0.15s', background: isOpen ? (dm ? 'rgba(37,99,235,0.05)' : '#f8faff') : 'transparent' }}>
                    <button
                      onClick={() => setOpen(isOpen ? null : i)}
                      style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', gap: 12 }}
                    >
          ...
          {/* Ticket form */}
          <div id="ticket" style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: '40px 48px', boxShadow: dm ? 'none' : '0 1px 4px rgba(0,0,0,0.05)' }}>
            <h2 style={{ fontFamily: F, fontSize: 18, fontWeight: 700, color: textPrim, margin: '0 0 6px', letterSpacing: '-0.02em' }}>
            ...
            {status === 'sent' ? (
              <div style={{ padding: '36px 28px', textAlign: 'center', border: `1px solid ${dm ? 'rgba(5,150,105,0.35)' : '#6ee7b7'}`, borderRadius: 12, background: dm ? 'rgba(5,150,105,0.06)' : '#f0fdf4' }}>
                ...
                <p style={{ fontFamily: F, fontSize: 13, color: textBody, marginBottom: 20, lineHeight: 1.7 }}>Our team will respond within 24–48 hours. Check your inbox for a confirmation email.</p>
              ...
              <form onSubmit={onSubmit} style={{ display: 'grid', gap: 16 }}>
                ...
                <div>
                  <button type="submit" disabled={status === 'sending'} style={{ background: status === 'sending' ? '#93c5fd' : '#2563eb', border: 'none', borderRadius: 8, padding: '13px 32px', color: '#fff', fontWeight: 700, fontSize: 12, letterSpacing: '0.08em', cursor: status === 'sending' ? 'not-allowed' : 'pointer', transition: 'background 0.2s', fontFamily: F }}>
