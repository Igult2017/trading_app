    ...
    <section id="reviews" className="py-24 bg-[#020817]">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          <span className="text-xs font-display font-bold tracking-[3px] uppercase text-blue-500 block mb-3">Testimonials</span>
          ...
          <p className="font-body text-slate-400 text-lg">Don&apos;t take our word for it.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {TESTIMONIALS.map((t, i) => (
            <motion.div key={i}
              ...
              <p className="font-body text-slate-300 text-[14px] leading-relaxed mb-5">&ldquo;{t.quote}&rdquo;</p>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${COLORS[i]} flex items-center justify-center text-white font-display font-bold text-sm shrink-0`}>
                  {t.name.charAt(0)}
                </div>
                <div>
                  <div className="font-display text-white font-semibold text-sm">{t.name} {t.flag}</div>
                  <div className="font-body text-slate-500 text-xs">{t.role}</div>
                </div>
              </div>
            </motion.div>
          ))}
