  ...
  return (
    <div className="relative rounded-2xl border border-white/10 bg-[#0f172a] p-4 shadow-[0_32px_80px_rgba(37,99,235,0.2)] overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-3">
        <span className="text-[10px] text-slate-500 font-mono tracking-widest">PORTFOLIO OVERVIEW</span>
      ...
      <div className="bg-blue-950/20 border border-white/[0.05] rounded-xl p-3 mb-3">
        <div className="text-[9px] text-slate-500 mb-2 font-mono">EQUITY CURVE (30D)</div>
      ...
      <div className="space-y-1.5">
        {TRADES.map((t) => (
          <div key={t.pair} className="flex justify-between items-center px-2.5 py-1.5 rounded-lg bg-blue-950/20 border border-white/[0.04]">
            <span className="text-[11px] font-semibold text-slate-200 font-mono">{t.pair}</span>
            <span className={`text-[9px] font-mono ${t.dir === "BUY" ? "text-emerald-400" : "text-red-400"}`}>{t.dir}</span>
            <span className={`text-[11px] font-bold font-mono ${t.win ? "text-emerald-400" : "text-red-400"}`}>{t.r}</span>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-14">
          ...
          <div className="flex-1 min-w-0 z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
              ...
              <span className="text-sm text-blue-300">Trusted by 10,000+ traders</span>
            </motion.div>

            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
              className="text-5xl lg:text-6xl font-extrabold text-white leading-[1.1] tracking-tight mb-5">
            ...
            <motion.p initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              className="text-slate-400 text-lg leading-relaxed mb-5 max-w-lg">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}
              ...
              {["Log trades", "Capture decisions", "Track psychology", "Build your edge"].map((item) => (
                <span key={item} className="flex items-center gap-1.5 text-xs font-mono text-blue-400">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}
              ...
              <a href="/auth?mode=signup" target="myfm_journal"
                className="inline-flex items-center gap-2 rounded-full bg-blue-600 hover:bg-blue-500 px-8 py-3.5 text-sm font-bold text-white shadow-[0_4px_20px_rgba(37,99,235,0.4)] hover:shadow-[0_8px_32px_rgba(37,99,235,0.5)] transition-all duration-300">
                Start Free — No Credit Card
              </a>
              <button onClick={() => document.getElementById("demo")?.scrollIntoView({ behavior: "smooth" })}
                className="inline-flex items-center gap-2 rounded-full border border-slate-600 hover:border-blue-500 px-7 py-3.5 text-sm font-semibold text-slate-300 hover:text-blue-400 transition-all duration-300">
                See How It Works ↓
              </button>
            </motion.div>
