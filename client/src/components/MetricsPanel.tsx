...
const Num = ({ children, size=17, color=P.green, style={} as React.CSSProperties }: any) => (
  <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:size, color, fontWeight:700, letterSpacing:'0.02em', lineHeight:1, ...style }}>{children}</span>
);
...
const YN = ({ label, yes, no }: { label:string; yes:number|null|undefined; no:number|null|undefined }) => (
  <div style={{ display:'flex', borderBottom:`1px solid ${P.line}`, alignItems:'stretch' }}>
    ...
    <div style={{ width:52, padding:'3px 5px', borderRight:`1px solid ${P.line}`, textAlign:'center' }}>
      ...
      {yes == null ? <Mono size={9} color={P.dim}>--</Mono> : <Cond size={13} color={pColor(yes)} weight={700}>{Math.round(yes)}%</Cond>}
    </div>
    <div style={{ width:52, padding:'3px 5px', textAlign:'center' }}>
      <Mono size={7} color={P.dim} style={{ display:'block', marginBottom:1 }}>NO</Mono>
      {no == null ? <Mono size={9} color={P.dim}>--</Mono> : <Cond size={13} color={pColor(no)} weight={700}>{Math.round(no)}%</Cond>}
    </div>
  </div>
);
    ...
    <div style={{ display:'grid', gridTemplateColumns:`repeat(${options.length},1fr)`, gap:1, background:P.line }}>
      {options.map((o,i) => (
        <div key={i} style={{ background:P.bg2, padding:'5px 4px', textAlign:'center' }}>
          ...
          {o.pct == null ? <Mono size={9} color={P.dim}>--</Mono> : <Cond size={13} color={pColor(o.pct)} weight={700}>{Math.round(o.pct)}%</Cond>}
        </div>
      ))}
    </div>
  </div>
    ...
    <div style={{ display:'grid', gridTemplateColumns:`repeat(${scores.length},1fr)`, gap:1, background:P.line }}>
      {scores.map((s,i) => (
        <div key={i} style={{ background:P.bg2, padding:'5px 3px', textAlign:'center' }}>
          ...
          {s.pct == null ? <Mono size={9} color={P.dim}>--</Mono> : <Cond size={13} color={pColor(s.pct)} weight={700}>{Math.round(s.pct)}%</Cond>}
              ...
              <div style={{ marginBottom:6 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:P.line }}>
                  {[['YES', boolImpacts.breakevenApplied?.yes?.winRate, P.green],['NO', boolImpacts.breakevenApplied?.no?.winRate, P.red]] .map(([l,v,c]:any,i)=>(
                    <div key={i} style={{ background:P.bg2, padding:'5px 4px', textAlign:'center' }}>
                      <Mono size={7} color={P.dim} style={{ display:'block' }}>{l}</Mono>
                      {v==null?<Mono size={9} color={P.dim}>--</Mono>:<Cond size={13} color={c} weight={700}>{Math.round(v)}%</Cond>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {candleEntries.length>0
              ? candleEntries.map((x,i)=>(
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'5px 0', borderBottom:`1px solid ${P.line}` }}>
                    ...
                    {x.wr==null ? <Mono size={9} color={P.dim}>--</Mono> : <Cond size={13} color={pColor(x.wr)} weight={700}>{x.wr}%</Cond>}
                  </div>
                ))
