    ...
    <footer style={{ background: bg, borderTop: `1px solid ${topBorder}`, fontFamily: "'Poppins',sans-serif", transition: 'background 0.3s' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 28px' }}>
        <div className="hf-grid">
          ...
          {/* ── Markets ────────────────────────────────────────────────── */}
          <div>
            <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: headLabel, marginBottom: 6 }}>Instruments</div>
            <h4 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 900, fontSize: 15, letterSpacing: '-0.01em', color: headTitle, marginBottom: 20, marginTop: 0 }}>Markets</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
              {MARKETS.map(({ label, href }) => (
                <a key={label} href={href} className="hf-lnk">{label}</a>
              ))}
            </div>
          </div>
